...
class TestDeleteChat():
    def test_if_user_is_anonymous_returns_401(self, call_delete_chat_endpoint):
        response = call_delete_chat_endpoint(1, 1)
        
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_if_user_is_not_admin_returns_403(self,
                                              authenticate_as_any,
                                              call_delete_chat_endpoint):
        authenticate_as_any()
    ...
    def test_if_user_is_admin_and_pc_id_doesnt_exist_returns_404(self,
                                                                 test_data,
                                                                 authenticate_as_any,
                                                                 call_delete_chat_endpoint):
        authenticate_as_any(is_staff=True)

        response = call_delete_chat_endpoint(test_data.INVALID_ID, 1)
        
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_if_user_is_admin_and_chat_id_doesnt_exist_returns_404(self,
                                                                   test_data,
                                                                   authenticate_as_any,
                                                                   create_private_chat,
                                                                   call_delete_chat_endpoint):
        ...
        response = call_delete_chat_endpoint(private_chat.id, test_data.INVALID_ID)
        
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_if_user_is_admin_and_chat_doesnt_belong_to_referenced_pc_returns_404(
        self,
        ...
        referenced_pc, other_pc = create_private_chats_for_user(users[0], users[1:])
        chat = create_chat(users[0], other_pc)

        response = call_delete_chat_endpoint(referenced_pc.id, chat.id)
        
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_if_user_is_admin_and_chat_belongs_to_referenced_pc_returns_204(
        self,
        authenticate_as_any,
        create_private_chat,
        create_chat,
        ...
        response = call_delete_chat_endpoint(private_chat.id, chat.id)
        
        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert not Chat.objects.filter(pk=chat.id).exists()


@pytest.mark.django_db
class TestCurrentChat():
    def test_if_user_is_anonymous_returns_401(self, call_current_chat_endpoint):
        response = call_current_chat_endpoint(1)
        
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
    
    def test_if_pc_id_doesnt_exist_returns_404(self,
                                               test_data,
                                               authenticate_as_any,
                                               call_current_chat_endpoint):
        authenticate_as_any()

        response = call_current_chat_endpoint(test_data.INVALID_ID)
    ...
    def test_if_user_is_not_pc_member_returns_403(self,
        ...
        response = call_current_chat_endpoint(private_chat.id)
    
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_if_user_is_pc_member_with_no_open_chat_returns_200(self,
                                                                authenticate_as_pc_member,
                                                                call_current_chat_endpoint):
        user, private_chat = authenticate_as_pc_member()
    ...
    def test_if_user_is_pc_member_with_open_chat_returns_200(self,
                                                             create_chat,
                                                             authenticate_as_pc_member,
                                                             call_current_chat_endpoint):
        user, private_chat = authenticate_as_pc_member()
        ...
        assert response.status_code == status.HTTP_200_OK
        assert (response.data['user'] == user.id and 
                response.data['terminated_at'] is None)
